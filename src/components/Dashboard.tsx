...
const Dashboard = ({ onBack, onNavigate }: DashboardProps) => {
  ...
  const [faqs] = useState<Array<{ q: string; a: string }>>([
    ...
    { q: 'Who can I contact for urgent help?', a: 'Use Emergency Helplines for immediate assistance.' }
  ])

  useEffect(() => {
    fetchDashboardData()
    // Realtime subscription to update stats live
    const channel = supabase
      .channel('citizen-dashboard')
